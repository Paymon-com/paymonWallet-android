...
NetworkManager::NetworkManager() {
    ...
    networkBuffer = new SerializedBuffer((uint32_t) READ_BUFFER_SIZE);
...
void NetworkManager::sendData(SerializedBuffer *buffer) {
    genericConnection->sendData(buffer, false);
}

int NetworkManager::callEvents(int64_t now) {
    if (!events.empty()) {
...
void NetworkManager::checkPendingTasks() {
    while (true) {
...
void NetworkManager::scheduleTask(std::function<void()> task) {
    pthread_mutex_lock(&mutex);
...
void NetworkManager::scheduleEvent(EventObject *eventObject, uint32_t time) {
...
void NetworkManager::removeEvent(EventObject *eventObject) {
...
void *NetworkManager::ThreadProc(void *data) {
    DEBUG_D("network thread started");
#ifdef ANDROID
    DEBUG_D("ATTACH");
    javaVm->AttachCurrentThread(&jniEnv, NULL);
#endif
    NetworkManager *networkManager = (NetworkManager *) (data);
...
//            DEBUG_D("SHIIIIIIIIIIIIIIIIIIIT");
...
//    }
//    SerializedBuffer *buffer = config->readConfig();
//    if (buffer != nullptr) {
...
//        config = new Config("tgnet.dat");
//    }
//    SerializedBuffer *buffer = BuffersStorage::getInstance().getFreeBuffer(32 * 1024);
...
//inline SerializedBuffer *decompressGZip(SerializedBuffer *data) {
//    int retCode;
//    z_stream stream;
//
...
//        exit(1);
//    }
//    SerializedBuffer *result = BuffersStorage::getInstance().getFreeBuffer(data->limit() * 4);
...
//        }
//        if (retCode == Z_OK) {
//            SerializedBuffer *newResult = BuffersStorage::getInstance().getFreeBuffer(result->capacity() * 2);
...
//inline SerializedBuffer *compressGZip(SerializedBuffer *buffer) {
...
//    }
//
//    SerializedBuffer *result = BuffersStorage::getInstance().getFreeBuffer(buffer->limit());
...
int64_t NetworkManager::getCurrentTimeMillis() {
    clock_gettime(CLOCK_REALTIME, &timeSpec);
    return (int64_t) timeSpec.tv_sec * 1000 + (int64_t) timeSpec.tv_nsec / 1000000;
}

int32_t NetworkManager::getCurrentTime() {
    return (int32_t) (getCurrentTimeMillis() / 1000) + timeDifference;
}

int32_t NetworkManager::getTimeDifference() {
    return timeDifference;
}

int64_t NetworkManager::generateMessageId() {
...
void NetworkManager::onConnectionClosed(Connection *connection) {
...
void NetworkManager::onConnectionConnected(Connection *connection) {
//    Datacenter *datacenter = connection->getDatacenter();
    if (connection->getConnectionType() == ConnectionTypeGeneric) {
...
//void NetworkManager::onConnectionQuickAckReceived(Connection *connection, int32_t ack) {
...
void NetworkManager::onConnectionDataReceived(Connection *connection, SerializedBuffer *data, uint32_t length) {
    delegate->onConnectionDataReceived(data, length);
}
//    bool error = false;
//    if (length == 4) {
...
//        }
//
//        Packet *object = TLdeserialize(request, messageLength, data);
//
//        if (object != nullptr) {
//            if (datacenter->isHandshaking()) {
...
//
//        if (!doNotProcess) {
//            Packet *object = TLdeserialize(nullptr, messageLength, data);
//            if (object != nullptr) {
...
//Packet *NetworkManager::getRequestWithMessageId(int64_t messageId) {
//    for (requestsIter iter = runningRequests.begin(); iter != runningRequests.end(); iter++) {
//        Request *request = iter->get();
...
Packet *NetworkManager::TLdeserialize(Packet *request, uint32_t bytes, SerializedBuffer *data) {
    bool error = false;
    ...
    Packet *object = ClassStore::PMdeserialize(data, bytes, constructor, error);
...
void NetworkManager::processServerResponse(Packet *message, int64_t messageId, int32_t messageSeqNo, int64_t messageSalt, Connection *connection, int64_t innerMsgId, int64_t containerMessageId) {
...
//        if (hasResult) {
//            Packet *object = response->result.get();
...
//                        SerializedBuffer *unpacked_data = nullptr;
//                        Packet *result = response->result.get();
...
//                            Packet *object = TLdeserialize(request->rawRequest, unpacked_data->limit(), unpacked_data);
//                            if (object != nullptr) {
//                                response->result = std::unique_ptr<Packet>(object);
//                            } else {
//                                response->result = std::unique_ptr<Packet>(nullptr);
//                            }
//                        }
//
...
//            networkMessage->message->body = std::unique_ptr<Packet>(request);
...
//        SerializedBuffer *data = decompressGZip(response->packed_data.get());
//        Packet *object = TLdeserialize(getRequestWithMessageId(messageId), data->limit(), data);
//        if (object != nullptr) {
//            DEBUG_D("connection(%p, dc%u, type %d) received object %s", connection, datacenter->getDatacenterId(), connection->getConnectionType(), typeid(*object).name());
...
//        } else {
//            if (delegate != nullptr) {
//                SerializedBuffer *data = BuffersStorage::getInstance().getFreeBuffer(message->getObjectSize());
...
//void NetworkManager::sendPing(Datacenter *datacenter, bool usePushConnection) {
...
//    std::vector<std::unique_ptr<NetworkMessage>> array;
//    array.push_back(std::unique_ptr<NetworkMessage>(networkMessage));
//    SerializedBuffer *transportData = datacenter->createRequestsData(array, nullptr, connection);
//    if (usePushConnection) {
//        DEBUG_D("dc%d send ping to push connection", datacenter->getDatacenterId());
//        sendingPushPing = true;
//    }
//    connection->native_sendData(transportData, false);
//}

bool NetworkManager::isIpv6Enabled() {
    return ipv6Enabled;
}

//void NetworkManager::initDatacenters() {
...
void NetworkManager::attachConnection(Socket *connection) {
...
void NetworkManager::detachConnection(Socket *connection) {
    std::vector<Socket *>::iterator iter = std::find(activeConnections.begin(), activeConnections.end(), connection);
    if (iter != activeConnections.end()) {
        activeConnections.erase(iter);
    }
}

//int32_t NetworkManager::sendRequestInternal(Packet *object, onCompleteFunc onComplete, onQuickAckFunc onQuickAck, uint32_t flags, uint32_t datacenterId, ConnectionType connetionType, bool immediate) {
...
//int32_t NetworkManager::sendRequest(Packet *object, onCompleteFunc onComplete, onQuickAckFunc onQuickAck, uint32_t flags, uint32_t datacenterId, ConnectionType connetionType, bool immediate) {
...
//void NetworkManager::sendRequest(Packet *object, onCompleteFunc onComplete, onQuickAckFunc onQuickAck, uint32_t flags, uint32_t datacenterId, ConnectionType connetionType, bool immediate, int32_t requestToken) {
//    if (!currentUserId && !(flags & RequestFlagWithoutLogin)) {
//        DEBUG_D("can't do request without login %s", typeid(*object).name());
//        delete object;
//        return;
...
#ifdef ANDROID
//void NetworkManager::sendRequest(Packet *object, onCompleteFunc onComplete, onQuickAckFunc onQuickAck, uint32_t flags, uint32_t datacenterId, ConnectionType connetionType, bool immediate, int32_t requestToken, jobject ptr1, jobject ptr2) {
//    if (!currentUserId && !(flags & RequestFlagWithoutLogin)) {
//        DEBUG_D("can't do request without login %s", typeid(*object).name());
//        delete object;
//        JNIEnv *env = 0;
...
//void NetworkManager::cancelRequestsForGuid(int32_t guid) {
//    scheduleTask([&, guid] {
...
//void NetworkManager::bindRequestToGuid(int32_t requestToken, int32_t guid) {
...
//void NetworkManager::setUserId(int32_t userId) {
//    scheduleTask([&, userId] {
...
//void NetworkManager::switchBackend() {
//    scheduleTask([&] {
...
//void NetworkManager::removeRequestFromGuid(int32_t requestToken) {
...
//bool NetworkManager::cancelRequestInternal(int32_t token, bool notifyServer, bool removeFromClass) {
//    for (requestsIter iter = requestsQueue.begin(); iter != requestsQueue.end(); iter++) {
//        Request *request = iter->get();
...
void NetworkManager::cancelRequest(int32_t token, bool notifyServer) {
    if (token == 0) {
        return;
    }
...
//void NetworkManager::onDatacenterHandshakeComplete(Datacenter *datacenter, int32_t timeDiff) {
//    saveConfig();
...
//void NetworkManager::onDatacenterExportAuthorizationComplete(Datacenter *datacenter) {
//    saveConfig();
//    processRequestQueue(AllConnectionTypes, datacenter->getDatacenterId());
//}

//void NetworkManager::sendMessagesToConnection(std::vector<std::unique_ptr<NetworkMessage>> &messages, Connection *connection, bool reportAck) {
...
//            SerializedBuffer *transportData = datacenter->createRequestsData(currentMessages, reportAck ? &quickAckId : nullptr, connection);
//
...
//                }
//
//                connection->native_sendData(transportData, reportAck);
//            } else {
...
//void NetworkManager::sendMessagesToConnectionWithConfirmation(std::vector<std::unique_ptr<NetworkMessage>> &messages, Connection *connection, bool reportAck) {
...
//void NetworkManager::requestSaltsForDatacenter(Datacenter *datacenter) {
...
//    request->num = 32;
//    sendRequest(request, [&, datacenter](Packet *response, TL_error *error) {
...
//void NetworkManager::clearRequestsForDatacenter(Datacenter *datacenter) {
//    for (requestsIter iter = runningRequests.begin(); iter != runningRequests.end(); iter++) {
//        Request *request = iter->get();
//        Datacenter *requestDatacenter = getDatacenterWithId(request->datacenterId);
...
//void NetworkManager::registerForInternalPushUpdates() {
...
//void NetworkManager::processRequestQueue(uint32_t connectionTypes, uint32_t dc) {
...
//            SerializedBuffer *original = BuffersStorage::getInstance().getFreeBuffer(requestLength);
//            request->rpcRequest->serializeToStream(original);
//            SerializedBuffer *buffer = compressGZip(original);
//            if (buffer != nullptr) {
...
//                request->rpcRequest = std::unique_ptr<Packet>(packed);
...
//                        }
//                        message->body = std::unique_ptr<Packet>(request);
//                        message->bytes += 4 + 8;
//                    }
//
...
//Datacenter *NetworkManager::getDatacenterWithId(uint32_t datacenterId) {
//    if (datacenterId == DEFAULT_DATACENTER_ID) {
...
//std::unique_ptr<Packet> NetworkManager::wrapInLayer(Packet *object, Datacenter *datacenter, Request *baseRequest) {
...
//            request->query = std::unique_ptr<Packet>(object);
...
//            request2->query = std::unique_ptr<Packet>(request);
//            DEBUG_D("wrap in layer %s", typeid(*object).name());
//            return std::unique_ptr<Packet>(request2);
//        }
//    }
//    return std::unique_ptr<Packet>(object);
//}

void NetworkManager::updateDcSettings(uint32_t dcNum) {
...
void NetworkManager::moveToDatacenter(uint32_t datacenterId) {
...
//        sendRequest(request, [&, datacenterId](Packet *response, TL_error *error) {
//            if (error == nullptr) {
...
void NetworkManager::authorizeOnMovingDatacenter() {
...
void NetworkManager::authorizedOnMovingDatacenter() {
...
void NetworkManager::applyDatacenterAddress(uint32_t datacenterId, std::string ipAddress, uint32_t port) {
...
void NetworkManager::setDelegate(ConnectiosManagerDelegate *connectiosManagerDelegate) {
    delegate = connectiosManagerDelegate;
}

//void NetworkManager::setPushConnectionEnabled(bool value) {
...
void NetworkManager::init(std::string host, const uint16 &port, uint32_t version, int32_t layer, int32_t apiId, std::string deviceModel, std::string systemVersion, std::string appVersion, std::string langCode, std::string configPath, std::string logPath, int32_t userId, bool isPaused, bool enablePushConnection) {
...
//    loadConfig();
    ...
    pthread_create(&networkThread, NULL, (NetworkManager::ThreadProc), this);
}

void NetworkManager::connect() {
    genericConnection = new Connection(ConnectionTypeGeneric);
    genericConnection->connect(host, port);
}
void NetworkManager::reconnect() {
    genericConnection->reconnect();
}

void NetworkManager::resumeNetwork(bool partial) {
...
void NetworkManager::setNetworkAvailable(bool value) {
    scheduleTask([&, value] {
...
void NetworkManager::setUseIpv6(bool value) {
    scheduleTask([&, value] {
        ipv6Enabled = value;
    });
}
...
#ifdef ANDROID
void NetworkManager::useJavaVM(JavaVM *vm, bool useJavaByteBuffers) {
    javaVm = vm;
