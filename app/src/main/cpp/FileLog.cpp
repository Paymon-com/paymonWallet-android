...
#ifdef ANDROID
    __android_log_vprint(ANDROID_LOG_ERROR, "paymon-jni-debug", message, argptr);
#else
...
void FileLog::w(const char *message, ...) {
    ...
    struct tm *now = localtime(&t);
#ifdef ANDROID
    __android_log_vprint(ANDROID_LOG_WARN, "paymon-jni-debug", message, argptr);
#else
...
void FileLog::d(const char *message, ...) {
    ...
    struct tm *now = localtime(&t);
#ifdef ANDROID
    __android_log_vprint(ANDROID_LOG_DEBUG, "paymon-jni-debug", message, argptr);
#else
