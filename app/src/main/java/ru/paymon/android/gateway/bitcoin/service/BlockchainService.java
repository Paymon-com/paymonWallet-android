...
public class BlockchainService extends LifecycleService {
    ...
    public static void start(final Context context, final boolean cancelCoinsReceived) {
        if (cancelCoinsReceived)
            context.startService(new Intent(BlockchainService.ACTION_CANCEL_COINS_RECEIVED, null, context, BlockchainService.class));
        else
    ...
    public static void resetBlockchain(final Context context) {
        ...
        context.startService(new Intent(BlockchainService.ACTION_RESET_BLOCKCHAIN, null, context, BlockchainService.class));
    }

    public static void broadcastTransaction(final Context context, final Transaction tx) {
        final Intent intent = new Intent(BlockchainService.ACTION_BROADCAST_TRANSACTION, null, context, BlockchainService.class);
        ...
        private void changed(final int numPeers) {
            ...
            handler.post(() -> {
                ...
                if (!connectivityNotificationEnabled || numPeers == 0) {
                    ...
                    final NotificationCompat.Builder notification = new NotificationCompat.Builder(BlockchainService.this, Constants.NOTIFICATION_CHANNEL_ID_ONGOING);
