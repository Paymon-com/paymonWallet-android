...
public class EmojiconsPopup extends PopupWindow implements ViewPager.OnPageChangeListener, EmojiconRecents, NotificationManager.IListener {
    ...
    public EmojiconContainer.OnEmojiconClickedListener onEmojiconClickedListener;
    ...
    public void setStickersListener(StickersListener stickersListener) {
        mEmojisAdapter.getStickersFragment().setOnStickerClickListener(stickersListener);
    }

    /**
     * Set the listener for the event when any of the emojicon is clicked
     */
    public void setOnEmojiconClickedListener(EmojiconContainer.OnEmojiconClickedListener listener) {
    ...
    void addStickerPack(StickerPack sp) {
    ...
    private View createCustomView() {
        ...
        view = inflater.inflate(R.layout.emojicons_popup, null, false);
        ...
        mEmojisAdapter = new EmojisPagerAdapter(
                Arrays.asList(
                        new EmojiconRecentsContainer(mContext, null, null, this, mUseSystemDefault),
                        new EmojiconContainer(mContext, People.DATA, recents, this, mUseSystemDefault),
                        new EmojiconContainer(mContext, Nature.DATA, recents, this, mUseSystemDefault),
                        new EmojiconContainer(mContext, Food.DATA, recents, this, mUseSystemDefault),
                        new EmojiconContainer(mContext, Sport.DATA, recents, this, mUseSystemDefault),
                        new EmojiconContainer(mContext, Cars.DATA, recents, this, mUseSystemDefault),
                        new EmojiconContainer(mContext, Electr.DATA, recents, this, mUseSystemDefault),
                        new EmojiconContainer(mContext, Symbols.DATA, recents, this, mUseSystemDefault),
                        new StickersContainer(mContext, this)
                )
        );
        ...
        mEmojiTabs[8] = view.findViewById(R.id.emojis_tab_stickers);
        ...
        for (int i = 0; i < mEmojiTabs.length; i++) {
            ImageButton btn = (ImageButton) mEmojiTabs[i];
        ...
        StickerPack sp = MediaManager.getInstance().loadStickerPack(1);
        if (sp != null) {
            mEmojisAdapter.getStickersFragment().addStickers(sp);
        }
    ...
    public void addRecentEmoji(Context context, Emojicon emojicon) {
        EmojiconRecentsContainer fragment = ((EmojisPagerAdapter) emojisPager.getAdapter()).getRecentFragment();
    ...
    public void didReceivedNotification(int id, Object... args) {
        if (id == NotificationManager.didLoadedStickerPack) {
            ...
            // Sticker Pack ID
            int spid = (Integer) args[0];

            StickerPack sp = MediaManager.getInstance().loadStickerPack(spid);
            if (sp != null) {
                mEmojisAdapter.getStickersFragment().addStickers(sp);
            }
        }
    ...
    private static class EmojisPagerAdapter extends PagerAdapter {
        private List<Container> views;

        public EmojiconRecentsContainer getRecentFragment() {
            for (Container it : views) {
                if (it instanceof EmojiconRecentsContainer)
                    return (EmojiconRecentsContainer) it;
            }
            return null;
        }

        public StickersContainer getStickersFragment() {
            for (Container it : views) {
                if (it instanceof StickersContainer)
                    return (StickersContainer) it;
            }
            return null;
        }

        public EmojisPagerAdapter(List<Container> views) {
            super();
            this.views = views;
        }
    ...
    public interface StickersListener {
        ...
        void onStickerSelected(int stickerPackID, long stickerID);
    }
}